...
The current template release is `v1.0.1`. Publish the public `tech-progress/railway-template-promptfoo` mirror at `release-v1` and tag `v1.0.1` before applying the graph. Both base images use immutable multi-architecture digests, only the Basic Auth gateway is public, and Promptfoo has a 5 GB persistent volume.
